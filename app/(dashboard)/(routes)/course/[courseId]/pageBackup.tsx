  ...
  const handleUserAuth = async () => {
    try {
      if (user && user.user && user.user.fullName) { // Verifica se há um usuário autenticado
        ...
        // Chama a função fetchRegisterUser para registrar o usuário
        await fetchRegisterUser(userId, fullName).then((data) => {
          console.log(data);
    ...
    <div className="w-full">
      ...
      <div className="w-full bg-dark-color text-white min-h-96 py-8 border-y ">
        ...
        {isLoading ? (
          ...
          <div className="flex mt-8 gap-32 flex-wrap justify-center">
            <SkeletonInstructorCard></SkeletonInstructorCard>
          </div>
        )}
...
export default CoursePageBackup;
